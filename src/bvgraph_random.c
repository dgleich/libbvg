 ...
 */
static int position_bvgraph(bvgraph_random_iterator *ri, long long x, unsigned long long *d)
{
    if (x < 0 || x >= ri->g->n) {
...
int bvgraph_random_successors(bvgraph_random_iterator *ri, 
                             long long x, long long** start, unsigned long long *length)
{
    if (x<0 || x >= ri->g->n) {
        return (bvgraph_vertex_out_of_range);
    }
        ...
        long long ref, ref_index;
        long long i, extra_count, block_count = 0;
        ...
        *length = d;
        ...
        long long* temp = NULL;
        long long* ref_links = NULL;
        unsigned long long outd_ref = 0;
        ...
        if (ref > 0) {
            ...
            ref_links = malloc(sizeof(long long)*outd_ref);
            memcpy(ref_links, temp, sizeof(long long)*outd_ref);
        ...
        if (ref > 0) {
            ...
            #ifdef MAX_DEBUG
                fprintf(stderr, "block_count = %llu\n", block_count);
            #endif 
        ...
        else
        {
            ...
            {
                long long j, cur_left = ri->left.a[i];
            ...
            if (extra_count > 0)
            {
                ...
                buf2_index = 0;           
                memcpy(ri->buf1.a, ri->successors.a, buf1_index*sizeof(long long));
            }
            else
            {
                memcpy(ri->buf1.a, ri->buf2.a, buf2_index*sizeof(long long));
        ...
        if (ref <= 0)
        {
            ...
            if (interval_count == 0 || extra_count == 0) {
                memcpy(ri->successors.a, ri->buf1.a, sizeof(long long)*buf1_index);
            }
        }
