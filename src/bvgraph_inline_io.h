...
static inline long long nat2int(const long long x) { return x % 2 == 0 ? x >> 1 : -( ( x + 1 ) >> 1 ); }

static inline unsigned long long read_coded(bitfile *bf, enum bvgraph_compression_flag_tag c) 
{
    switch (c) {
...
static inline unsigned long long read_offset(bvgraph *g, bitfile *bf) { return read_coded(bf, g->offset_coding); }
static inline unsigned long long read_outdegree(bvgraph *g, bitfile *bf) { return read_coded(bf, g->outdegree_coding); }
static inline unsigned long long read_residual(bvgraph *g, bitfile *bf) 
{ 
...
static inline unsigned long long read_reference(bvgraph *g, bitfile *bf) { return read_coded(bf, g->reference_coding); }
static inline unsigned long long read_block(bvgraph *g, bitfile *bf) { return read_coded(bf, g->block_coding); }
static inline unsigned long long read_block_count(bvgraph *g, bitfile *bf) { return read_coded(bf, g->block_count_coding); }
